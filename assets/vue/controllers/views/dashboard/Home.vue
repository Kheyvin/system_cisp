  ...
  <div>
    ...
    <div class="grid grid-cols-12 gap-6">
      ...
      <div class="col-span-12 md:col-span-6 lg:col-span-3">
        <Card class="h-full flex flex-col p-6 hover:shadow-lg transition-shadow duration-200 bg-principal text-white min-h-[200px] justify-between">
          ...
          <div class="grow flex flex-col justify-end">
            <h3 class="text-2xl font-bold text-white mb-1">{{ connectionStatusText }}</h3>
      ...
      <!-- Card 2: Proximo Pago -->
      <div class="col-span-12 md:col-span-6 lg:col-span-3">
        <Card class="h-full flex flex-col p-6 hover:shadow-lg transition-shadow duration-200 min-h-[200px] justify-between bg-white">
          ...
          <div class="grow flex flex-col justify-end">
            <h3 class="text-2xl font-bold text-gray-800 mb-1">{{ planPrice }}</h3>
      ...
      <div class="col-span-12 lg:col-span-6">
        <Card class="h-full flex flex-col p-6 bg-white shadow-sm hover:shadow-lg transition-shadow duration-200">
          <div class="mb-4">
            <h3 class="text-lg font-bold text-gray-800">Plan Actual</h3>
            <p class="text-sm text-gray-500">Detalles de tu servicio de internet</p>
          </div>
          ...
          <div class="space-y-3 mb-6 grow">
            ...
            <div class="flex justify-between items-center border-b border-gray-100 pb-2">
              ...
              <span class="px-2 py-1 rounded text-white text-sm bg-[#324D68]">{{ planSpeed }}</span>
            </div>
            <div class="flex justify-between items-center border-b border-gray-100 pb-2">
            ...
            <div class="flex justify-between items-center">
              ...
              <span :class="statusClasses" class="px-2 py-1 rounded text-white text-xs font-bold uppercase">
                {{ status }}
              </span>
            </div>
          </div>

          <Button class="w-full bg-[#324D68] text-white hover:bg-[#24384c] py-2 rounded-md transition-colors duration-200">
            Cambiar plan
          </Button>
        </Card>
      </div>
...
const currentSpeedDisplay = computed(() => {
  switch(status.value) {
    case 'activo': return measuredSpeed.value;
    case 'cargando': return '0 Mbps';
    case 'inactivo': return 'Error de Red';
    case 'suspendido':
    case 'cancelado': return 'Regularizar Pago';
    default: return '---';
  }
});
...
const currentSpeedClass = computed(() => {
  switch(status.value) {
    case 'activo': return 'text-2xl font-bold text-gray-800 mb-1';
    case 'cargando': return 'text-2xl font-bold text-gray-400 mb-1';
    case 'inactivo': return 'text-lg font-bold text-red-600 bg-red-100 px-2 py-1 rounded mb-1 inline-block rounded-xl';
    case 'suspendido':
    case 'cancelado': return 'text-lg font-bold text-orange-600 bg-orange-100 px-2 py-1 rounded mb-1 inline-block rounded-xl';
    default: return 'text-2xl font-bold text-gray-800 mb-1';
  }
});
...
const wifiIconClass = computed(() => {
  return status.value === 'activo' ? 'text-green-300' : 'text-red-300';
});
